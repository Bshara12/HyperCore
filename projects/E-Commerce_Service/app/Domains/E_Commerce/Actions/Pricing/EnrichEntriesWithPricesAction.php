...
class EnrichEntriesWithPricesAction
{
  ...
  public function execute(array $entries): array
  {
    ...
    return collect($entries)->map(function ($entry) use ($autoPrices, $userPrices) {
      ...
      // 🔹 stock: extracted by field name, not data_type_field_id — EAV field IDs
      // are not guaranteed stable across data types/projects.
      $stockField = isset($entry['values'][0])
        ? collect($entry['values'])->firstWhere('field.name', 'count')
        : null;

      $availableStock = $stockField !== null ? (int) $stockField['value'] : null;
