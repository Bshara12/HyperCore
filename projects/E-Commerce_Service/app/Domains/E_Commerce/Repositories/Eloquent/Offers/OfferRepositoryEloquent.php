...
class OfferRepositoryEloquent implements OfferRepositoryInterface
{
  ...
  public function getOfferDetails($collectionId): Offer
  {
    // 🟢 تكييش تفاصيل العرض باستعمال Tag العروض ومفتاح الكاش الموحد
    return Cache::tags(['offers'])->remember(
      CacheKeys::offer((int) $collectionId),
      now()->addHours(24),
      fn() => Offer::where('collection_id', $collectionId)->firstOrFail()
    );
  }
  ...
  {
    return Cache::tags(['offers'])->remember(
      CacheKeys::offers((int) $projectId),
      now()->addHours(24),
      fn() => Offer::where('project_id', $projectId)->get()
    );
  }
