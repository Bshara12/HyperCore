...
class AuthApiClientTest extends TestCase
{
  ...
  protected function setUp(): void
  {
    ...
    Config::set('services.auth.url', $this->fakeUrl);
  ...
  #[Test]
  public function it_fetches_service_details_successfully(): void
  {
    $token = 'valid-token';
    $fakeServiceData = ['id' => 'service-123', 'name' => 'My Service'];

    Http::fake([
      "{$this->fakeUrl}/api/get-service" => Http::response(['data' => $fakeServiceData], 200)
    ]);

    $result = $this->authClient->getServiceFromToken($token);

    $this->assertEquals($fakeServiceData, $result);
  }

  #[Test]
  public function it_throws_exception_when_service_fetch_fails_with_json(): void
  {
    $token = 'invalid-token';
    $errorMessage = 'Service unavailable';

    Http::fake([
      "{$this->fakeUrl}/api/get-service" => Http::response(['message' => $errorMessage], 403)
    ]);

    // نستخدم try-catch لضمان وصول الكود لسطر الـ throw الخاص بك
    try {
      $this->authClient->getServiceFromToken($token);
      $this->fail('Exception was not thrown');
    } catch (\Exception $e) {
      $this->assertStringContainsString('Failed to fetch user from auth service: ' . $errorMessage, $e->getMessage());
    }
  }

  #[Test]
  public function it_throws_exception_when_service_fetch_fails_with_raw_body(): void
  {
    $token = 'error-token';
    $rawError = 'Service Error Raw';

    Http::fake([
      "{$this->fakeUrl}/api/get-service" => Http::response($rawError, 500)
    ]);
    ...
    try {
      ...
      $this->assertStringContainsString('Failed to fetch user from auth service: ' . $rawError, $e->getMessage());
    }
  }
}
