...
class EloquentBookingAnalyticsRepository implements AnalyticsRepositoryInterface
{
  ...
  //   $diffMinutesSql = $this->timestampDiffMinutes('b.start_at', 'b.end_at');
  ...
  //                   THEN {$diffMinutesSql} END), 2), 0) as avg_duration_minutes
  //           ")
  ...
  //   $availHoursSql = $this->timestampDiffHours('start_time', 'end_time');
  ...
  //     ->selectRaw("
  //               resource_id,
  //               SUM({$availHoursSql}) as daily_hours
  //           ")
  //     ->groupBy('resource_id')
  ...
  public function getResourcePerformance(AnalyticsFilterDTO $dto): array
  {
    ...
    $resources = DB::table('resources as r')
      ...
      ->selectRaw("
            ...
            COALESCE(SUM(CASE WHEN b.status NOT IN ('cancelled') THEN b.amount ELSE 0 END), 0) as total_revenue,
            COALESCE(SUM(CASE WHEN b.status = 'cancelled' THEN b.refund_amount ELSE 0 END), 0) as total_refunded,
            COALESCE(ROUND(AVG(CASE WHEN b.status NOT IN ('cancelled') THEN {$diffMinutesSql} END), 2), 0) as avg_duration_minutes
        ")
      ->groupBy('r.id', 'r.name', 'r.type', 'r.capacity', 'r.payment_type', 'r.price')
      ->orderByRaw('total_bookings DESC')
      ->get();
    ...
    // حساب الـ Occupancy Rate لكل resource
    $totalDays = Carbon::parse($dto->from)->diffInDays(Carbon::parse($dto->to)) + 1;
    ...
    // جيب الساعات الفعلية المحجوزة لكل resource (مع مراعاة التوافقية مع SQLite)
    $bookedHoursSql = $this->timestampDiffHours('start_at', 'end_at');
    ...
    $bookedHours = DB::table('bookings')
      ...
      ->selectRaw("
            resource_id,
            COALESCE(SUM({$bookedHoursSql}), 0) as booked_hours
        ")
      ->groupBy('resource_id')
      ->pluck('booked_hours', 'resource_id');
    ...
    return [
      ...
      'resources' => $resources->map(function ($r) use ($totalDays, $availabilities, $bookedHours) {
        ...
        return [
          ...
          'avg_duration_minutes' => (float) $r->avg_duration_minutes,
          'cancellation_rate' => $totalBookings > 0
            ? round(($r->cancelled / $totalBookings) * 100, 2) : 0.0,
          'occupancy_rate' => $occupancyRate,
          'total_available_hours' => $totalAvailableHours,
          'total_booked_hours' => $totalBookedHours,
        ];
  ...
  public function getPeakTimes(AnalyticsFilterDTO $dto): array
  {
    ...
    $diffHoursSql = $this->timestampDiffHours('b.created_at', 'b.start_at');
    ...
    $avgLeadTime = DB::table('bookings as b')
      ...
      ->selectRaw("
                r.id                                                            as resource_id,
                r.name,
                ROUND(AVG({$diffHoursSql}), 2)                            as avg_lead_time_hours            ")
      ->groupBy('r.id', 'r.name')
  ...
  private function timestampDiffMinutes(string $fromColumn, string $toColumn): string
  {
    if (DB::connection()->getDriverName() === 'sqlite') {
      $from = "case when length({$fromColumn}) <= 8 then '2000-01-01 ' || {$fromColumn} else {$fromColumn} end";
      $to = "case when length({$toColumn}) <= 8 then '2000-01-01 ' || {$toColumn} else {$toColumn} end";

      return "((CAST(strftime('%s', {$to}) AS INTEGER) - CAST(strftime('%s', {$from}) AS INTEGER)) / 60.0)";
    }

    return "TIMESTAMPDIFF(MINUTE, {$fromColumn}, {$toColumn})";
  }

  private function timestampDiffHours(string $fromColumn, string $toColumn): string
  {
    if (DB::connection()->getDriverName() === 'sqlite') {
      $from = "case when length({$fromColumn}) <= 8 then '2000-01-01 ' || {$fromColumn} else {$fromColumn} end";
      $to = "case when length({$toColumn}) <= 8 then '2000-01-01 ' || {$toColumn} else {$toColumn} end";

      return "((CAST(strftime('%s', {$to}) AS INTEGER) - CAST(strftime('%s', {$from}) AS INTEGER)) / 3600.0)";
    }

    return "TIMESTAMPDIFF(HOUR, {$fromColumn}, {$toColumn})";
  }
}
