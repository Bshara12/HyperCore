...
class InsertCollectionItemsRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    $collection = DataCollection::where('project_id', CurrentProject::id())
      ->where('slug', $this->route('collectionSlug'))
      ->firstOrFail();
    ...
    return [
      ...
      'items.*' => [
        ...
        'integer',
        'distinct',
        Rule::exists('data_entries', 'id')
          ->where('project_id', $collection->project_id)
          ->where('data_type_id', $collection->data_type_id),
      ],
    ];
  }
