...
class TypoCorrector
{
  ...
  private function correctWord(string $word): array
  {
    ...
    foreach (self::KNOWN_WORDS as $known) {
      $knownLen = mb_strlen($known, 'UTF-8');

      if (abs($knownLen - $wordLen) > $maxDist) {
        continue;
      }
      ...
      if ($dist > 0 && $dist <= $maxDist && $dist < $bestDist) {
        ...
        // ✅ FIX #7: إضافة minimum similarity threshold
        // يمنع false positives مثل "note"→"nokia" (similarity=0.40)
        $maxLen     = max($wordLen, $knownLen);
        $similarity = $maxLen > 0 ? 1.0 - ($dist / $maxLen) : 0.0;

        if ($similarity < 0.60) {
          continue; // ← تجاهل إذا التشابه < 60%
        }
