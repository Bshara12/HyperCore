...
class EloquentPopularSearchRepository implements PopularSearchRepositoryInterface
{
  ...
  public function recompute(int $projectId, string $language): array
  {
    ...
    // استخدام صيغة معيارية للوقت (ISO) متوافقة مع محرك SQLite و MySQL للاختبارات بيسر
    $minus24h = now()->subHours(24)->toDateTimeString();
    $minus7d  = now()->subDays(7)->toDateTimeString();
    $minus30d = now()->subDays(30)->toDateTimeString();
    ...
    $stats = DB::select("
            SELECT
                ...
                language,
                SUM(CASE WHEN searched_at >= '{$minus24h}' THEN 1 ELSE 0 END) AS count_24h,
                SUM(CASE WHEN searched_at >= '{$minus7d}'  THEN 1 ELSE 0 END) AS count_7d,
                SUM(CASE WHEN searched_at >= '{$minus30d}' THEN 1 ELSE 0 END) AS count_30d,
