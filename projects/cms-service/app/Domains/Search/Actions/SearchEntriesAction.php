...
class SearchEntriesAction
{
  ...
  private function executeWithKeyword(
    ...
    // ✅ FIX #3: حذف $hasNegation من الشرط
    // النفي أُعولج بالفعل في normalizeQuery()
    // إضافته هنا تُشغّل AI بلا سبب على queries مثل "ما بدي ايفون 14"
    $needsFallback = $result['total'] <= $threshold
      || $isGibberish
      || empty($processed->cleanWords);
    // ❌ كان هكذا (خاطئ):
    ...
    if ($needsFallback) {
      ...
      // ── 5A. Keyboard Fix ──────────────────────────────────────
      // ✅ FIX #5: أضف !$isGibberish للحماية من تحويل gibberish
      // Gibberish لا يستفيد من keyboard fix ويُنتج arabic عشوائي
      if (! $isArabic && ! $isMixed && ! $isGibberish) {
        $kbResult = $this->tryKeyboardFix($dto, $preference, $result, false);
        if ($kbResult !== null) {
          $result        = $kbResult['result'];
          $processed     = $kbResult['processed'];
          $keyboardFixed = true;
