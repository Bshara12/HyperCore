...
final class SearchEntriesAction
{
    ...
    private function generateSnippet(string $content, array $words, int $before = 60, int $after = 100): string
    {
        ...
        // الكلمات مُطبَّعة والنص خام → مُطابقة متسامحة بالتشكيل/الهمزات
        $pos = null;
        foreach ($words as $word) {
            $p = $this->findLoosePosition($plain, (string) $word);
    ...
    private function highlightText(string $text, array $words): string
    {
        ...
        foreach ($words as $word) {
            ...
            // looseRegex بدل preg_quote: الكلمة "ايفون" يجب أن تُظلّل
            // "آيفون" في العنوان الخام كما يعرضه للمستخدم.
            $pattern = ArabicTextNormalizer::looseRegex((string) $word);
            $result  = preg_replace('/(?<!\*\*)(' . $pattern . ')(?!\*\*)/iu', '**$1**', $text);
            $text    = $result ?? $text;
        }
        return $text;
    }
    ...
    /**
     * موقع كلمة مُطبَّعة داخل نص خام (بالأحرف لا بالبايتات)
     */
    private function findLoosePosition(string $haystack, string $word): ?int
    {
        if ($word === '' || mb_strlen($word, 'UTF-8') < 2) {
            return null;
        }

        $pattern = '/' . ArabicTextNormalizer::looseRegex($word) . '/iu';

        if (preg_match($pattern, $haystack, $matches, PREG_OFFSET_CAPTURE) !== 1) {
            return null;
        }

        // preg_match يُرجع byte offset — نُحوّله لعدد أحرف
        return mb_strlen(substr($haystack, 0, $matches[0][1]), 'UTF-8');
    }
}